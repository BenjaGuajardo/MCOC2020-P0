...
+ Análisis
  ...
  + A medida que aumenta el N, entre N=10 y N=100, se puede notar que existen ciertos peaks. Lo anterior se puede asociar a que comienza a usarse un mayor porcentaje de  procesadores, lo cual explicaria las mejora en el desempeño, disminuyendo los tiempos de ejecucion.
  + Ya para los N mas grandes, se puede notar de mejor manera cual es el solver mas eficiente y rapido. Como era de esperarse, A_invB_inv es por lejos el mas lento. Para el resto de los solvers se puede notar que npSolve no sigue siendo el mas rapido, como en un comienzo. En cuanto a los solvers de scipy, el desempeño tambien es el esperado, spSolve es el mas lento, cuando especificamos que A es simetrica y definida positiva, el rendimiento mejora, y cuando se ocupa overwrite = True, el desempeño vuelve a mejorar. Lo anterior era lo esperado ya que al agregar estas especificaciones se optimiza el calculo, por ejemplo, cuando se señalamos que A es simetrica, el solver toma los valores del triangulo superior puesto que la matriz es simetrica. Tambien mediante overwrite, mejoramos la performance al permitir sobrescribir la matriz A.
